...
import pe.edu.pucp.a20152252.myfirstlogin.data.api.ApiAdapter;
...
public class LoginPresenter implements ILoginPresenter {
    ...
    public void loginRest(final String username, final String password) {
        LoginInRO loginInRO = new LoginInRO(ApiAdapter.APPLICATION_NAME, username,
                password);
        Call<UserOutRO> call = ApiAdapter.getInstance().login(loginInRO);
        call.enqueue(new Callback<UserOutRO>() {
            @Override
            public void onResponse(@NonNull Call<UserOutRO> call, @NonNull
                    Response<UserOutRO> response) {
    ...
    private void processUserResponse(Response<UserOutRO> response, String username,
                                     String password) {
